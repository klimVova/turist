  ...
  <div>
    <div class="row"> <!-- блок, который переключается (боковой фильтр + посты) -->
      ...
      <div class="catalog-list col col-12 col-md-10">
        <div class="list">
          <div v-for="tur in turs" class="item fadeInRight wow " data-wow-offset='0'
               data-wow-delay='0.2s'
             ...
             || cities === tur.city_id && districts === tur.district_id && republics === tur.republic_id
              ...
              <router-link :to="'/tur/'+ tur.id">
                ...
                <div class="list-item-descr">
                  ...
                  <p>{{ tur.desc }}</p>
                  <ul>
                    <li v-for="tag in tur.tags">{{tag.title}}</li>
                  </ul>
                  <div class="company">
                    <img class="logo" :src="tur.logo_url">
                    <a :href="tur.link"><p>{{tur.name_link}}</p></a>
                  </div>
                </div>
              </router-link>
            </div>
...
export default {
  ...
  methods: {
    ...
    getFilterList() {
      this.axios.get('/api/turs/filters',)
          .then(res => {
