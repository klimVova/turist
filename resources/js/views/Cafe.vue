  ...
  <div>
    <div class="row"> <!-- блок, который переключается (боковой фильтр + посты) -->
      ...
      <div class="catalog-list col col-12 col-md-10">
        <div class="list">
          <div v-for="cafe in cafes" class="item fadeInRight wow " data-wow-offset='0'
               data-wow-delay='0.2s'
            ...
            <div v-if="
             ...
             || cities === cafe.city_id && districts === cafe.district_id && republics === cafe.republic_id
              ">
              <router-link class="d-flex" :to="'/cafe/'+ cafe.id">
              <div class="item-img">
              ...
              <div class="list-item-descr">
                ...
                <p>{{ cafe.desc }}</p>
                <ul>
                  <li v-for="tag in cafe.tags">{{tag.title}}</li>
                </ul>
                <div class="company">
                  <img class="logo" :src="cafe.logo_url">
                  <a :href="cafe.link"><p>{{cafe.name_link}}</p></a>
                </div>
              </div>
              </router-link>
              </div>
