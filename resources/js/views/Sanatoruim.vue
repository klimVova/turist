  ...
  <div>
    <div class="row"> <!-- блок, который переключается (боковой фильтр + посты) -->
      ...
      <div class="catalog-list col col-12 col-md-10">
        <div class="list">
          <div v-for="sanatorium in sanatoriums" class="item fadeInRight wow " data-wow-offset='0'
               data-wow-delay='0.2s'
               data-wow-duration='1s'>
            <div v-if="
             state.city === sanatorium.city_id
             && state.district === sanatorium.district_id
             && state.republic === sanatorium.republic_id
             ...
             || cities === sanatorium.city_id && districts === sanatorium.district_id && republics === sanatorium.republic_id
              ...
              <router-link class="d-flex" :to="'/sanatorium/'+ sanatorium.id">
                ...
                <div class="list-item-descr">
                  ...
                  <p>{{ sanatorium.desc }}</p>
                  <ul>
                    <li v-for="tag in sanatorium.tags">{{tag.title}}</li>
                  </ul>
                  <div class="company">
                    <img class="logo" :src="sanatorium.logo_url">
                    <a :href="sanatorium.link"><p>{{sanatorium.name_link}}</p></a>
                  </div>
                </div>
              </router-link>
            </div>
...
export default {
  ...
  methods: {
    ...
    getFilterList() {
      this.axios.get('/api/sanatoriums/filters',)
          .then(res => {
