...
<div class="content-wrapper">
    ...
    <section class="content">
        <div class="container-fluid">
            ...
            <div class="row">
                <div class="col-12">
                    <form action="{{route('turoperator.todoitem.update', $turoperatorTodoItem->id)}}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PATCH')
                        <div class="form-group w-25">
                            <input type="text" class="form-control" name="title" placeholder="Название подкатегории\процедуры"
                                   value="{{$turoperatorTodoItem->title}}">
                            @error('title')
                        ...
                        <div class="form-group w-25">
                            <input type="text" class="form-control" name="price" placeholder="Цена"
                                   value="{{$turoperatorTodoItem->price}}">
                            @error('title')
                            <div class="text-danger">{{$message}}</div>
                            @enderror
                        </div>
