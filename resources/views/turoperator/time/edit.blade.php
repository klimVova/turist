...
<div class="content-wrapper">
    ...
    <section class="content">
        <div class="container-fluid">
            ...
            <div class="row">
                <div class="col-12">
                    <form action="{{route('turoperator.time.update', $turoperatorTime->id)}}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PATCH')
                        <div class="form-group w-25">
                            <input type="text" class="form-control" name="time_start" placeholder="Начало работы центра"
                                   value="{{$turoperatorTime->time_start}}">
                            @error('time_start')
                        ...
                        <div class="form-group w-25">
                            <input type="text" class="form-control" name="time_end" placeholder="Конец работы центра"
                                   value="{{$turoperatorTime->time_end}}">
                            @error('time_end')
                            <div class="text-danger">{{$message}}</div>
                            @enderror
                        </div>
                        <div class="form-group w-25">
                            <input type="text" class="form-control" name="time_date" placeholder="Дни работы центра"
                                   value="{{$turoperatorTime->time_date}}">
                            @error('time_date')
                            <div class="text-danger">{{$message}}</div>
                            @enderror
                        </div>
