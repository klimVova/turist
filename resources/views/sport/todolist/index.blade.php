    ...
    <div class="content-wrapper">
        ...
        <div class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1 class="m-0"> Инфраструктура\инвентарь </h1>
                    </div><!-- /.col -->
        ...
        <section class="content">
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-9">
                        <form action="{{route('sport.todolist.store')}}" method="POST" class="w-55">
                            @csrf
                        ...
                        @if(count($sportTodoLists))
                            <ul style="background:antiquewhite;" class="list-group mt-3">
                                @foreach($sportTodoLists as $sportTodoList)
                                    <li class="list-group-item justify-content-sm-between d-flex">
                                        <div class="col">
                                            ...
                                            <div class="d-flex justify-content-xl-between">
                                                <div><h3>{{$sportTodoList->title}}</h3></div>
                                                <div class="d-flex">
                                                    <a href="{{route('sport.todolist.edit', $sportTodoList->id)}}"
                                                       class='text-success mr-5'>
                                                        <i class="fas fa-pencil-alt"></i></a></td>
                                                    <form action="{{route('sport.todolist.delete', $sportTodoList->id)}}"
                                                          method="post">
                                                        @csrf
                                                        @method('DELETE')
                                                    ...
                                                    <a style="width: 50px;height: 40px;"
                                                       href="{{route('sport.todolist.item', $sportTodoList->id)}}">
                                                        <button type="submit" class="btn btn-dark btn-sm px-4">
                                    ...
                                    <table class="table">
                                        <thead>
                                        <tr>
                                            <th scope="col">Подкатегория// инфраструктура\инвентарь</th>
                                            <th colspan="3" class="text-center">Действие</th>
                                        </tr>
                                        </thead>
                                        <tbody>
                                        @foreach($items as $item)
                                            @if($sportTodoList->id == $item->sport_todo_list_id)
                                                <tr>
                                                    <td>
                                                        <div class="d-flex">
                                                            <h3>{{$item->title}}</h3>
                                                        ...
                                                        @foreach($products as $product)
                                                            @if($item->id == $product->sport_todo_item_id)
                                                                <div class="d-flex align-items-baseline ml-5">
                                                                    ...
                                                                    <p class="pl-5 align-items">
                                                                        <a href="{{route('sport.todoproduct.edit', $product->id)}}"
                                                                           class='text-success'>Редактировать</a>
                                                                    </p>
                                                                    <p class='pl-5'>
                                                                    <form action="{{route('sport.todoitem.product_remove', $product->id)}}"
                                                                          method="post">
                                                                        @csrf
                                                                        @method('DELETE')
                                                                        <button type="submit"
                                                    ...
                                                    <td><a href="{{route('sport.todoitem.edit', $item->id)}}"
                                                           class='text-success'><i class="fas fa-pencil-alt"></i></a>
                                                    </td>
                                                    <td>
                                                        <form action="{{route('sport.todolist.item_remove', $item->id )}}"
                                                              method="post">
                                                            @csrf
                                                            @method('DELETE')
                                                    ...
                                                    <td>
                                                        <a style="width: 300px;height: 40px;"
                                                           href="{{route('sport.todoitem.product', $item->id)}}">
                                                            <button type="submit" class="btn btn-dark btn-sm px-4">
                                                                <i class="fas fa-plus"></i></button>
                                                        </a>
                                                    </td>
