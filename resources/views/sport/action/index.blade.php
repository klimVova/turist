    ...
    <div class="content-wrapper">
        ...
        <section class="content">
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-9">
                        <form action="{{route('sport.action.store')}}" method="POST" class="w-55">
                            @csrf
                        ...
                        @if(count($sportActions))
                            <ul style="background:antiquewhite;" class="list-group mt-3">
                                @foreach($sportActions as $sportAction)
                                    <li class="list-group-item justify-content-sm-between d-flex">
                                        <div class="col">
                                            <div class="d-flex justify-content-xl-between">
                                                <div><p>{{$sportAction->title}}</p></div>
                                                <div class="d-flex">
                                                    <a href="{{route('sport.action.edit', $sportAction->id)}}"
                                                       class='text-success mr-5'>
                                                        <i class="fas fa-pencil-alt"></i></a>
                                                    <form action="{{route('sport.action.delete', $sportAction->id)}}"
                                                          method="post">
                                                        @csrf
                                                        @method('DELETE')
